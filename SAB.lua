...
local shop = player.PlayerGui:FindFirstChild('Main')
    and player.PlayerGui.Main:FindFirstChild('CoinsShop')

local Window = Fluent:CreateWindow({
    Title = game:GetService('MarketplaceService')
        :GetProductInfo(109983668079237).Name .. ' 〢 Jatrogzz',
    SubTitle = 'https://discord.gg/kbGDJHSqmE',
    TabWidth = 160,
...
local remainingTime =
    workspace.Plots[plotName].Purchases.PlotBlock.Main.BillboardGui.RemainingTime
local rtp =
    Tabs.Main:AddParagraph({ Title = 'Lock Time: ' .. remainingTime.Text })
        ...
        while character and hum and hum.Parent do
            if currentSpeed > 0 and hum.MoveDirection.Magnitude > 0 then
                character:TranslateBy(
                    hum.MoveDirection * currentSpeed * hb:Wait() * 10
                )
            end
            task.wait()
        end
...
Tabs.Main:AddButton({
    ...
    Callback = function()
        ...
        if cloak and cloak:GetAttribute('SpeedModifier') == 2 then
            ...
            Fluent:Notify({
                Title = 'Stellar',
                Content = 'Use Invisibility Cloak First',
                Duration = 2,
            })
        end
    end,
...
local function updatelock()
    ...
    for _, plot in pairs(workspace.Plots:GetChildren()) do
        local timeLabel = plot:FindFirstChild('Purchases', true)
            ...
            and plot.Purchases.PlotBlock.Main:FindFirstChild(
                'BillboardGui',
                true
            )
            and plot.Purchases.PlotBlock.Main.BillboardGui:FindFirstChild(
                'RemainingTime',
                true
            )
        ...
        if timeLabel and timeLabel:IsA('TextLabel') then
            ...
            local displayText = isUnlocked and 'Unlocked'
                or ('Lock: ' .. timeLabel.Text)
            ...
            for _, child in pairs(plot:GetDescendants()) do
                ...
                then
                    ...
                    local existingBillboard =
                        parentModel:FindFirstChild(espName)
                    local existingMutationBillboard =
                        parentModel:FindFirstChild(mutationEspName)
                    ...
                    if activeESP[child.Text] then
                        ...
                        then
                            ...
                            if not existingMutationBillboard then
                                ...
                                mutationLabel.TextColor3 =
                                    mutationSettings.Color
                                mutationLabel.TextStrokeColor3 =
                                    Color3.new(0, 0, 0)
                                ...
                                existingMutationBillboard.TextLabel.Text =
                                    mutation.Text
                                existingMutationBillboard.TextLabel.TextColor3 =
                                    mutationSettings.Color
                            end
...
Tabs.Main:AddKeybind('Keybind', {
    ...
    Callback = function(Value)
        shop.Visible = Value
        shop.Position = Value and UDim2.new(0.5, 0, 0.5, 0)
...
local petModels = game:GetService('ReplicatedStorage').Models.Animals
    :GetChildren()
...
local function getOwner(plot)
    local text = plot:FindFirstChild('PlotSign')
            ...
            and plot.PlotSign.SurfaceGui.Frame.TextLabel.Text
        or 'Unknown'
                ...
                for _, plot in pairs(workspace.Plots:GetChildren()) do
                    if plot.Name ~= myPlotName then
                        ...
                        for _, v in pairs(plot:GetDescendants()) do
                            if
                                v.Name == 'DisplayName'
                                and table.find(SelectedPets, v.Text)
                            then
                                counts[owner] = counts[owner] or {}
                                counts[owner][v.Text] = (
                                    counts[owner][v.Text] or 0
                                ) + 1
                                found = true

                                if
                                    not lastResults[owner]
                                    or not lastResults[owner][v.Text]
                                then
                                    newPetsFound = true
                                end
                            end
...
Tabs.Server:AddButton({
    ...
    Callback = function()
        ...
        local File = pcall(function()
            AllIDs = game:GetService('HttpService')
